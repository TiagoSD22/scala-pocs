...
class HTab(var tab: Array[Option[HNode]] = Array.fill(0)(None), var mask: Int = 0, var size: Int = 0) {
  ...
  def hForeach(f: (HNode, Any) => Boolean, arg: Any): Boolean = {
    for (i <- tab.indices) {
      var node = tab(i)
      while (node.isDefined) {
        if (!f(node.get, arg)) return false
        node = node.get.next
      }
    }
    true
...
class HMap {
  ...
  def hmForeach(f: (HNode, Any) => Boolean, arg: Any): Boolean = {
    newer.hForeach(f, arg) && older.hForeach(f, arg)
  }
}
